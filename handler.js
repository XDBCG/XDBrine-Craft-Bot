...
module.exports = {
    async handler(chatUpdate) {
        ...
        try {
            ...
            const isRFounder = global.rfounder.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isFounder = isRFounder || global.founder.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isCoFounder = isFounder || global.cofounder.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            if (!isCoFounder && db.data.settings.self) return // Saat mode self diaktifkan hanya Founder dam Co-Founder yang dapat menggunakannya
            const isOwner = isCoFounder || global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isCoOwner = isOwner || global.coowner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isMods = isCoOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isStaff = isMods || global.staff.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            const isPrems = isStaff || db.data.users[m.sender].premium || false
            //let isPrems = isStaff || global.prems.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
            ...
            const isAdmin = isCoFounder || user && user?.admin || false // Is User Admin?
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (plugin.rfounder && isRFounder) { // Real Founder
                        fail('rfounder', m, this)
                        continue
                    }
                    if (plugin.founder && !isFounder) { // Founder
                        fail('founder', m, this)
                        continue
                    }
                    if (plugin.cofounder && !isCoFounder) { // Co-Founder
                        fail('cofounder', m, this)
                        continue
                    }
                    ...
                    if (plugin.coowner && !isCoOwner) { // Co-Owner
                        fail('coowner', m, this)
                        continue
                    }
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            if (e.name)
                            for (let jid of rfounder.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid)) {
   ...
   await conn.sendButtonDoc(id, text, wm, action == 'add' ? 'selamat datang' : 'sampai jumpa', action === 'add' ? '.intro' : 'XDBrine Craft', fake,{
  contextInfo: { externalAdReply :{
    showAdAttribution: true,
    mediaUrl: 'https://youtu.be/KuEfHlmMBOI',
    mediaType: 2,
...
`.trim(), msg, {
 ...
 global.dfail = async (type, m, conn) => {
    let msg = {
    	founder: 'Perintah ini hanya dapat digunakan oleh _*Founder Bot*_ !',
        cofounder: 'Perintah ini hanya dapat digunakan oleh _*Co-Founder Bot*_ !',
        owner: 'Perintah ini hanya dapat digunakan oleh _*Owner Bot*_ !',
        coowner: 'Perintah ini hanya dapat digunakan oleh _*Co-Owner Bot*_ !',
        mods: 'Perintah ini hanya dapat digunakan oleh _*Moderator Bot*_ !',
        staff: 'Perintah ini hanya dapat digunakan oleh _*Staff*_ !',
